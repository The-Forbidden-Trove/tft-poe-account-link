...
const notifyModmailLink = async (discordUserId) => {
  const guild = await client.guilds.fetch(TFT_SERVER_ID, true);
  let userChannel = guild.channels.cache.find(channel => channel.type == 0 && channel.parentId && channel.parentId == MODMAIL_CATEGORY && channel.topic && channel.topic.search(discordUserId) != -1);
